...
import com.mycompany.tetris2.Shape.Tetrominoes;
...
public class Board extends JPanel implements ActionListener {
  ...
  // Thêm biến để lưu giữ thông tin thời gian, số điểm và level
  ...
  // Thêm phương thức để tính toán thời gian chơi
  private int getGameTime() {
    return gameTime / 1000;
  }
  
  // Thêm phương thức để tính toán tốc độ game dựa trên level
  ...
  // Xử lý khi người chơi ấn phím "nhanh/chậm"
  ...
  private void pause() {
    ...
    if (isPaused) {
      ...
      timer.start();
      statusbar.setText("level: " + level + " - lines: " + numLinesRemoved + " - time: " + getGameTime() + "s");
    }
...
private void doDrawing(Graphics g) {
  ...
  private void removeFullLines() {
    ...
    if (numFullLines > 0) {
      ...
      // Tăng level và giảm thời gian giữa các lượt rơi
      level += numFullLines / 10;
      timer.setDelay(getTimerDelay());
